 ...
 * e.c
 * Friday, 6/13/1997.
 *         8/3/98 - convert to state machine
 *
 ...
 *  state description:
 *  ------------------
 *  Each animal contains an array of up to n states
 *  states have either:
 *    A> a condition and a next state for TRUE and a next state for FALSE
 *    B> an action, the next state will always be the TRUE state
 *  During a turn, states will be traveled until either n states have been
 *  traveled or an action takes place.
 *
 *  State:
 *  ------
 ...
 *  <action>               <object>              --------   <dir>          
 *  --------               ----------              a b c    -----       
 *  E      - eat           1   - anything        d e f g h   701       
 *  T<dir> - turn          0   - nothing         i j k l m   6.2       
 *  F      - move forward  F   - any edible      n o p q r   543      
 *  B      - move back     P   - any preditor      s . t       
 *  S      - sleep         L   - any flora         u v w              
 *                         A   - any fauna
 *                         ~c  - flora/fauna obj with tag 'c'
 *
 *  <location>       <dist>        <side>
 *  -------------    ------        ------   
 *  <mappos>         C - close     R - right
 *  <dist><side>     M - medium    L - left 
 *                   F - far       F - front
 *                   A - any       B - back 
 *                                 A - any
 *  Condition BNF
 *  -------------
 *  A[E|F|B|S|T#]
 *  O(1|0|F|P|L|A|~c)([a-w]|([CMFA][RLFBA]))
 *  F#
 *  R##
 *
 *
